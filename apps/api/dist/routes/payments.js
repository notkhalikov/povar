...
async function paymentsRoutes(app) {
    ...
    app.post('/payments/telegram-webhook', {
        schema: {
            body: {
                type: 'object',
        ...
        const [payment] = await app.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.payments.orderId, orderId), (0, drizzle_orm_1.eq)(schema_js_1.payments.status, 'created')))
            .orderBy(schema_js_1.payments.createdAt)
            .limit(1);
        if (!payment) {
        ...
        if (order.status !== 'awaiting_payment') {
            ...
            if (order.status === 'paid')
                return reply.send({ ok: true, alreadyPaid: true });
            return reply.code(422).send({ error: `Order is in unexpected status "${order.status}"` });
        }
        // Mark payment as paid
        await app.db
            .update(schema_js_1.payments)
            .set({
            status: 'paid',
            providerTxId: telegramPaymentChargeId,
            updatedAt: new Date(),
        })
            .where((0, drizzle_orm_1.eq)(schema_js_1.payments.id, payment.id));
        ...
        const [customer] = await app.db
            ...
            .limit(1);
        if (chef) {
            (0, notify_js_1.notifyOrderPaid)(order, chef.telegramId)
        ...
        if (chef && customer) {
            (0, notify_js_1.scheduleAutoComplete)(order, customer.telegramId, chef.telegramId, async () => {
                const [dispute] = await app.db
                    .select({ id: schema_js_1.disputes.id })
                ...
                const [current] = await app.db
                    .select({ status: schema_js_1.orders.status })
                    .from(schema_js_1.orders)
                    .where((0, drizzle_orm_1.eq)(schema_js_1.orders.id, orderId))
                    .limit(1);
                if (!current || !['paid', 'in_progress'].includes(current.status))
                    return;
                await app.db
                    .update(schema_js_1.orders)
                ...
                await app.db
                    ...
                    .set({ ordersCount: (0, drizzle_orm_1.sql) `orders_count + 1` })
